...
A small, pretty charting library for Swing. Pure JDK — zero dependencies — with a one-line API,
interactive charts by default, and a colourblind-aware palette validated for light and dark surfaces.

Every example below is real: the code shown is exactly what rendered the image next to it (the pairs are
regenerated by `ReadmeExamplesTest` on every build, so they can't drift apart).
...
`build/chart-*.png` — the render-and-look gate behind every change — and `ReadmeExamplesTest`, which
regenerates every image in this README from the exact code shown above.
