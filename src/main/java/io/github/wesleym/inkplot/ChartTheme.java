...
/**
 * Every colour a chart draws with, as one immutable value. The built-in {@link #light()} and {@link #dark()}
 * themes are a validated, colourblind-aware default (the dark series palette is the same eight hues re-stepped
 * for the dark surface, not an automatic flip) — pass a custom instance to restyle everything at once.
 *
...
public record ChartTheme(boolean dark, Color surface, Color text, Color muted, Color hairline,
	...
	/** The built-in dark theme: the same eight hues, re-stepped for the dark surface. */
