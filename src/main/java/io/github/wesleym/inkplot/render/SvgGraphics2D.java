...
/**
 * A minimal, dependency-free {@link Graphics2D} that records the drawing a chart renderer does into a vector
 * SVG document: solid fills, stroked shapes, text, alpha composites, and clipping (the plot area). Text is
 * emitted as outlined glyph paths by default, so the export carries no font dependency and never reflows on a
 * viewer lacking the font — in pure JDK; pass {@code textAsText} for compact real {@code <text>} instead.
 */
public final class SvgGraphics2D extends Graphics2D {
	...
	private Shape clip;
	private String clipRef = "";   // the clip-path attribute for elements drawn under the current clip, or ""
	 ...
	 */
	public SvgGraphics2D(int width, int height, boolean textAsText) {
		this.width = width;
		this.height = height;
	...
	public void fill(Shape s) {
		...
		body.append("<path d=\"").append(d).append("\" fill=\"").append(paintRef())
				.append("\" fill-opacity=\"").append(paintOpacity()).append('"').append(fillRule(s)).append(clipRef)
				.append("/>\n");
	}
	...
	public void draw(Shape s) {
		...
		body.append("<path d=\"").append(d).append("\" fill=\"none\" ").append(strokeAttrs()).append(clipRef)
				.append("/>\n");
	}
	...
	public void drawString(String str, float x, float y) {
		...
		if (textAsText) {
			...
			body.append("<text x=\"").append(n(p.getX())).append("\" y=\"").append(n(p.getY()))
					...
					.append("\" fill-opacity=\"").append(n(a)).append('"').append(clipRef).append('>')
					.append(escapeText(str)).append("</text>\n");
			return;
		}
	...
	// ---- clip (enforced: elements drawn under a clip carry a clip-path to a registered clipPath) ------
	...
	public void clip(Shape s) {
		...
		clip = clip == null ? s : intersect(clip, s);
		updateClip();
	}

	private static Shape intersect(Shape a, Shape b) {
		java.awt.geom.Area area = new java.awt.geom.Area(a);
		area.intersect(new java.awt.geom.Area(b));
		return area;
	}

	// Registers the current clip (transformed to root coordinates, userSpaceOnUse) as a <clipPath> and points
	// subsequent elements at it via a clip-path attribute; a null clip clears it.
	private void updateClip() {
		if (clip == null) {
			clipRef = "";
			return;
		}
		String d = pathData(clip);
		if (d.isEmpty()) {
			clipRef = "";
			return;
		}
		int id = ++gradientId[0];
		defs.append("<clipPath id=\"c").append(id).append("\" clipPathUnits=\"userSpaceOnUse\"><path d=\"")
				.append(d).append("\"/></clipPath>\n");
		clipRef = " clip-path=\"url(#c" + id + ")\"";
	}
